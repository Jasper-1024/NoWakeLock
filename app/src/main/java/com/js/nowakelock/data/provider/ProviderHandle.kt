...
//        LogUtil.d("Xposed.NoWakeLock", "Bundle1 ${tmp} ${tmp.size()}")
    ...
    fun test(bundle: Bundle): Bundle? {
        val test = bundle.get("Test") as Test?
        LogUtil.d("Xposed.NoWakeLock", "PH1 $test")

        val tmp = Bundle()

        val temp = Test("test", "package", 1, 1, 1, 1)

//        tmp.putParcelable("Test", temp)
        tmp.putSerializable("Test", temp)
        LogUtil.d("Xposed.NoWakeLock", "PH2 $tmp")
