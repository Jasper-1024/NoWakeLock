...
//    suspend fun loadInfo(name: String, userId: Int = 0): Info?
    ...
    @Query("update info set count = count+:count where name_info = :name and type_info = :type and userId = :userId")
    ...
    @Query("update info set blockCount = blockCount+:count where name_info = :name and type_info = :type and userId = :userId")
    ...
    @Query("update info set countTime = countTime+:time where name_info = :name and type_info = :type and userId = :userId")
    suspend fun upCountTime(time: Long, name: String, type: Type, userId: Int = 0)


    @Query("update info set count = 0 where type_info = :type and userId = :userId")
    suspend fun rstAllCount(type: Type, userId: Int = 0)

    @Query("update info set countTime = 0 where type_info = :type and userId = :userId")
